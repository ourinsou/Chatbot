# -*- coding: utf-8 -*-
"""
Created on Sun Apr 11 12:22:27 2021
...
#Load the intents
nlp = StanfordCoreNLP(r'C:\Users\sword\Downloads\Test_Synonym\stanford-corenlp-4.2.0')
...
def send():
            ...
            if msg != '':
                ...
                if emotion == "Negative" and res[1] == "noanswer" and res[2] == '':
                    sorry = trans("zh", " : I am sorry to hear that ")
                    ChatLog.insert(END, sorry)
                    ChatLog.image_create(END, image = sad)
                    flickr = trans("zh","\nHere is an image from flickr: ")
                    ChatLog.insert(END,flickr)
                    ChatLog.insert(END," " + msg + "\n")
                    app.flickr_search(msg)
                    ChatLog.insert(END, "\n\n")
                    ...
                    if res[2] != "":
                        ...
                        ChatLog.insert(END," " + trans("zh",res[0]) + "\n")
                        wiki = trans("zh","This is what I found on Wikipedia about ")
                        srch = trans("zh",search) 
                        ChatLog.insert(END,wiki + srch + ":\n")
                        
                        try:
                            ChatLog.insert(END,trans("zh",wikipedia.summary(res[2][0], sentences =3)))
                            flickr = trans("zh","\nHere is an image from flickr: ")
                            ChatLog.insert(END,flickr)
                            app.flickr_search(msg)
                           # global search_img
                    ...
                    ChatLog.insert(END,flickr)
                    ChatLog.insert(END," " + msg + "\n")
                    app.flickr_search(search)
                    ChatLog.insert(END, "\n\n" + msg + "\n\n")
                elif emotion == "Negative" and res[1] == "noanswer" and res[2] != '':
                    ChatLog.insert(END, " : " )
                    ChatLog.image_create(END, image = confused)
                    res[0] = res[0].replace("%",res[2][0])
                    ChatLog.insert(END," " + trans("zh",msg) + "\n")
                    wiki = trans("zh","This is what I found on Wikipedia about ")
                    flickr = trans("zh","\nHere is an image from flickr: ")
                    ChatLog.insert(END,wiki + res[2][0] + ":\n")
                    ChatLog.insert(END, trans("zh",wikipedia.summary(res[2][0], sentences =3)))
                    ChatLog.insert(END,flickr)
                    ChatLog.insert(END," " + msg + "\n")
                    app.flickr_search(search)
